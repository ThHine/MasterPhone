...
//    private String imageUrl; // khai báo biến imageUrl ở đây
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        v = inflater.inflate(R.layout.editproduct_fragment, container, false);
...
//        medtSoLuongSP = v.findViewById(R.id.edtSoLuong);
        ...
        mbtnCapNhat = v.findViewById(R.id.btnCapNhat);
...
//        String image = bundle.getString("image");
        ...
        Glide.with(v.getContext()).load(image).into(mimgSP);
...
//                imageUri = data.getData();
                Glide.with(editProductFragment.this).load(imageUri)
...
//        Glide.with(addFragment.this).load(imageUri).override(100, 100).centerCrop().into(mimgSP);
        if (imageUri != null) {
            ...
            myRef.putFile(imageUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    ...
                    myRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            if (uri != null) {
                                ...
                                Toast.makeText(editProductFragment.this.getActivity(), "Không tìm thấy uri", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(editProductFragment.this.getActivity(), "Lỗi lấy đường link ảnh", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(editProductFragment.this.getActivity(), "Lỗi tải ảnh lên Firebase Storage", Toast.LENGTH_SHORT).show();
                }
            });
        }
                    ...
                    myRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            if (uri != null) {
                                imageURL = uri.toString();
                                if (imageURL == null) {
                                    Toast.makeText(editProductFragment.this.getActivity(), "Chưa tìm thấy đường dẫn hình ảnh", Toast.LENGTH_SHORT).show();
                                } else {
...
//                    }
                                    collectionReference.document(id).update(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            Toast.makeText(editProductFragment.this.getActivity(), "Thêm thành công", Toast.LENGTH_SHORT).show();
                                            getParentFragmentManager().popBackStack();
                                        }
                                    }).addOnFailureListener(new OnFailureListener() {
                                        @Override
                                        public void onFailure(@NonNull Exception e) {
                                            Toast.makeText(editProductFragment.this.getActivity(), "Thêm thất bại", Toast.LENGTH_SHORT).show();
                                        }
                                    });
                                }
                            } else {
